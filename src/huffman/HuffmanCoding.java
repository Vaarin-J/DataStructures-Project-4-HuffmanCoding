...
public class HuffmanCoding {
    ...
    public void makeEncodings() {
       ...
       for(int i =0; i<sortedCharFreqList.size(); i++){
            ...
            traversal(temp, huffmanRoot, "");
       }        
        
    }
    private boolean traversal(CharFreq Char, TreeNode root, String output){
        int ascii;
        if(Char.getCharacter()!=null && root.getData().getCharacter() != null && root.getData().getCharacter().equals(Char.getCharacter())){
            ascii = (int)Char.getCharacter();
            encodings[ascii] = output;
        }

        if(root.getLeft() != null){
            if(traversal(Char, root.getLeft(), output+"0")){
                return true;
            }
        }
        if(root.getRight() != null){
            if(traversal(Char, root.getRight(), output+"1")){
                return true;
            }
        }
        return false;
    ...
    public void encode(String encodedFile) {
        ...
        while(StdIn.hasNextChar()){
            ...
            int number = (int) c;
            encoding = encoding + encodings[number];
        }
    ...
    public void decode(String encodedFile, String decodedFile) {
        ...
        String[] arr = new String[bit.length()];

        for(int i = 0; i<arr.length; i++){
            arr[i]= bit.substring(i, i+1);
            System.out.println(arr[i]);
        }

        for(int i=0; i<arr.length;i++){
            if(arr[i].equals("0")){
                //System.out.println(temp.getData().getCharacter());
                temp = temp.getLeft();
                //System.out.println(temp.getData().getCharacter());
            }
            else{
